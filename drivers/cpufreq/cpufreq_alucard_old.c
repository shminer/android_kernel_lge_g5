...
static int cpufreq_governor_alucard_old(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	struct cpufreq_alucard_policyinfo *ppol;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ALUCARD_OLD
static
#endif
struct cpufreq_governor cpufreq_gov_alucard_old = {
	.name = "alucard_old",
	.governor = cpufreq_governor_alucard_old,
...
static int __init cpufreq_alucard_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_alucard_old);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ALUCARD_OLD
...
static void __exit cpufreq_alucard_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_alucard_old);
